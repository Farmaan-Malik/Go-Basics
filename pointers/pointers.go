...
func changeAgeToAdultyears(number *int) {
	...
	// Additionally, any argument passed to a function is a copy of the original data unless a pointer is used
	// which means changing it inside a function wouldn't change the original data. When a pointer is passed, only then
	// can we change the original data because we are accessing its memory location directly.
